...
def generate_random_mini_batch_seeds_list(OUTPUT_NID, args):
	...
	# print(OUTPUT_NID.tolist())
	# map_output_list = list(map(OUTPUT_NID.__getitem__, indices))
	...
	# print('OUTPUT_NID.tolist()')
	# print(OUTPUT_NID.tolist())
	# print(list(batches_nid_list))

	# batches_nid_list = list(chunk(OUTPUT_NID.tolist(), mini_batch))
...
def check_connection_nids(given_output_nids, full_batch_subgraph):
	...
	f_g_dst_nid_list_ = full_batch_subgraph.dstdata['_ID'].tolist()
	given_nid_list_ = given_output_nids.tolist()



	for i in range(len(given_nid_list_)):
		if given_nid_list_[i] not in f_g_dst_nid_list_:
			print(given_nid_list_[i])
			print('given nid not in given****************************************************************************')

	# local_given_output_nids = [f_g_nid_list_.index(nid) for nid in given_nid_list_]
	# print(local_given_output_nids)

	# t21 = time.time()
	#
	# print('                          global nid to local graph nid old')
	# print(str(t21 - t1) + ' sec')
	# t22 = time.time()
	# f_g_nid_list = full_batch_subgraph.ndata['_ID']
	# local_given_output_nids_2 = [(f_g_nid_list == nid).nonzero().flatten() for step, nid in enumerate(given_output_nids)]
	...
	tn1 = time.time()
	print('local_given_output_nids')
	if None in local_given_output_nids:
		print('local_given_output_nids error-----------------------------------------------------------------------------------------------$$$$$')

	# print(local_given_output_nids)
	print(str(tn1 - tn) + ' sec')

	#
	# t2 = time.time()
	# print(local_given_output_nids)
	# print('                          global nid to local graph nid new ')
	# print(str(t2-tn1) + ' sec')


	t2 = time.time()
	local_in_edges_tensor = full_batch_subgraph.in_edges(local_given_output_nids, form='all')
	t31 = time.time()
	print(str(t31 - t2) + ' sec')
	...
	t32 = time.time()
	print(str(t32 - t31) + ' sec')
	srcid_list = list(numpy.array(nids_global_list)[srcid_local_list])
	t33 = time.time()
	print(str(t33 - t32) + ' sec')
	t3 = time.time()

	print('                         get local srcnid and dst nid')
	print(str(t3 - t2) + ' sec')
	...
	# tt1 = time.time()
	# eid_list = list(map(full_batch_subgraph.edata['_ID'].__getitem__, eid_local_list))
	# # print(eid_list)
	# #
	tt2 = time.time()
	...
	print('                 map local srcnid , dstnid,  eid to global')
	print(str(t4 - t3) + ' sec')
	print()
...
def check_connection_nids_1(given_output_nids, full_batch_subgraph):
	# global nid to local graph nid
...
def generate_blocks(G, full_batch_sub_graph, batches_nid_list):
	...
	for step, nids in enumerate(batches_nid_list):
		...
		print('time of check_connection_nids : ' + str(t12 - t11) + ' sec')


		cur_block = generate_one_block(G, connected_global_eid)
		t13 = time.time()
		print('after generate_one_block ')
		print('time of generate_one_block : ' + str(t13 - t12) + ' sec')
		# print('after generate_one_block')
		data_loader.append((srcnid, dstnid, [cur_block]))

	t1 = time.time()
	print('after generate blocks')
	print('time of generate_blocks generation : ' + str(t1 - tt) + ' sec')
...
def generate_dataloader( G, full_batch_data_blocks, args):
	...
	for bb in full_batch_data_blocks:
		...
		# print(eidx)
		block_to_graph = dgl.block_to_graph(bb)
		print('block to graph')
		...
		print(len(bb.ndata[dgl.NID]['_N']))
		print()
		print('full_batch_sub_graph.ndata[dgl.NID]')
		print(current_block_graph.ndata)
		...
		# # print(sub_graph_total_nid.size())
		full,_=torch.sort(current_block_graph.ndata[dgl.NID])
		block_, _=torch.sort(bb.ndata[dgl.NID]['_N'])

		if torch.equal(full, block_):
			...
			temp = current_block_graph.dstdata
			print('full batch graph dst dst data  vs full batch blocks')
			print(full_batch_data_blocks[-1].dstdata)
			print(temp)
			...
			data_loader = generate_blocks(G, current_block_graph, batches_nid_list)
			return data_loader
