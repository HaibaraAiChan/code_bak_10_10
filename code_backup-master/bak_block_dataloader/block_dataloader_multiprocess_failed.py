...
def check_connection_nids(given_output_nids, full_batch_block_graph):
...
def worker(shared_queue,lock, output_nid, dict_nid_2_local, full_batch_block_graph,block_eids_global_list,block_src_nid_list):
	print('start a worker-----------------------------------')
	# dict_nid_2_local, full_batch_block_graph,block_eids_global_list,block_src_nid_list= args_
	
	
	given_nid_list_ =  list(numpy.concatenate(output_nid).flat)
	print('given_nid_list_')
	print(given_nid_list_)
	
	print('dict_nid_2_local')
	# print(dict_nid_2_local)
	print()
	local_given_output_nids = list(map(dict_nid_2_local.get, given_nid_list_))
	...
	local_in_edges_tensor = torch.tensor([1,2,3,4], dtype=torch.long)
	...
	# get local srcnid and dstnid from subgraph
	mini_batch_srcid_local_list = list(local_in_edges_tensor)[0].tolist()
	print('mini_batch_srcid_local_list')
	print(mini_batch_srcid_local_list)
	srcid_list = list(numpy.array(block_src_nid_list)[mini_batch_srcid_local_list])
	print('srcid_list')
	print(srcid_list)
	...
	global_eid_tensor = torch.tensor(eid_list, dtype=torch.long)
	srcid = torch.tensor(list(set(given_nid_list_+ srcid_list)), dtype=torch.long)
	print('len(srcid), len(output_nid), len(global_eid_tensor)')
	print(len(srcid))
	print(len(output_nid))
	print(len(global_eid_tensor))

	shared_queue.put((srcid, output_nid, global_eid_tensor))
	print('len(shared_queue)')
	print(len(shared_queue))

def worker_2(name, que):
    que.put("%d is done" % name)



def check_connections(batch_nodes_list, full_batch_sub_graph):
	global full_batch_block_graph 
	full_batch_block_graph = full_batch_sub_graph
	manager = Manager()
	...
	for i in range(P_NUM):
		print('the process of ')
		print(i)
		# p.apply_async(worker_2, (33, shared_queue))
		p.apply_async(worker, args=(shared_queue,lock, batch_nodes_list[int(size / P_NUM) * i: int(size / P_NUM) * (i + 1)], dict_nid_2_local, full_batch_block_graph,block_eids_global_list,block_src_nid_list,))
		# p.apply_async(worker, args=(shared_queue, batch_nodes_list[int(size / P_NUM) * i: int(size / P_NUM) * (i + 1)], args_,))
	...
	i=0
	print('print shared queue')
	res=[]
	while not shared_queue.empty():
		cur = shared_queue.get()
		print(cur)
		res.append(cur)
	print('--------------------------------res-------------------------------')
	print(res)
...
# 	print('-----------------------------batches_temp_res_list-----------------------------------')
# 	print(batches_temp_res_list)
# 	if len(batches_temp_res_list)==0:
# 		print("ERROR***********")
# 		return
...
def worker_3(shared_queue,nids,raw_graph,full_batch_block_2_graph,check_connection_time,block_generation_time):
	print('coming into workers 3')
	t1= time.time()
	srcnid, dstnid, current_block_global_eid = check_connection_nids(nids, full_batch_block_2_graph)
	t2 = time.time()
	print('after check_connection_nids')
	check_connection_time.put(t2-t1) #------------------------------------------

	cur_block = generate_one_block(raw_graph, current_block_global_eid, srcnid)
	t3=time.time()
	block_generation_time.put(t3-t2)  #------------------------------------------
	
	shared_queue.put((srcnid, dstnid, [cur_block]))
...
def generate_blocks(raw_graph, full_batch_block_2_graph, batches_nid_list):
	...
	# block_generation_time = []
	manager = Manager()
	check_connection_time=manager.Queue()
	block_generation_time=manager.Queue()
	
	shared_queue=manager.Queue()
	lock = Manager().Lock()
	print(shared_queue)
	size = len(batches_nid_list)
	...
	for i in range(P_NUM):
		...
		print(i)
		# p.apply_async(worker_2, (33, shared_queue))
		p.apply_async(worker_3, args=(shared_queue,batches_nid_list[int(size / P_NUM) * i: int(size / P_NUM) * (i + 1)],raw_graph, full_batch_block_2_graph,check_connection_time,block_generation_time))
		# p.apply_async(worker, args=(shared_queue, batch_nodes_list[int(size / P_NUM) * i: int(size / P_NUM) * (i + 1)], args_,))
		# print('shared_queue queue+++++++++++++++++++++++++++++')
		# print(shared_queue)
...
def generate_dataloader(raw_graph, full_batch_data_blocks, args):
	for cur_block in full_batch_data_blocks:
		...
		if torch.equal(full, block_):  # make sure the new generated graph is exact the same with the current block
			...
			data_loader = generate_blocks(raw_graph, block_to_graph, batches_nid_list)
