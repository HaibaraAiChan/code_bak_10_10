...
def check_connection_nids(given_output_nids, edges, eidx):
	...
	for i in range(len(given_nid_list_)):
		if given_nid_list_[i] not in raw_dst:
			print(given_nid_list_[i])
			print('given nid not in given****************************************************************************')
	eidx_list = eidx.tolist()
	print('edix_list')
	# print(eidx_list)
	if None in eidx_list:
		print('error-------------------------------------------------------------------------------------edix_list------$$$$$')


	dict_ = {raw_dst[i]: eidx_list[i] for i in range(0, len(raw_dst))}
